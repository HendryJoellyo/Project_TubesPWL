    ...
    public function store(Request $request)
{
    ...
    $request->validate([
        ...
        'email' => 'required|email|unique:mahasiswa_profiles,email|unique:users,email', // Cek juga di users
        'password' => 'required|confirmed|min:6',
        'prodi_id' => 'required|exists:prodi,id',
    ]);

    // Simpan mahasiswa ke dalam mahasiswa_profiles
    $mahasiswa = MahasiswaProfile::create([
        'nrp' => $request->nrp,
        'name' => $request->name,
        'tanggal_lahir' => $request->tanggal_lahir,
        'email' => $request->email,
        'password' => bcrypt($request->password), // Mungkin tidak dipakai kalau login pakai tabel user
        'prodi_id' => $request->prodi_id,
    ]);

    // Simpan pengguna ke dalam users
    User::create([
        'name' => $request->name,
        'email' => $request->email,
        'password' => bcrypt($request->password),
        'role_id' => 8, // Role ID mahasiswa
    ]);

    Log::info('Data berhasil disimpan:', $mahasiswa->toArray());
