...
{
    // Ambil NIK dosen dan ID prodi yang sudah digunakan
    $usedNik = KetuaProdiProfile::pluck('nik')->toArray();
    $usedProdiId = KetuaProdiProfile::pluck('prodi_id')->toArray();

    // Ambil dosen yang NIK-nya belum digunakan
    $dosens = DosenProfile::whereNotIn('nik', $usedNik)->get();

    // Ambil prodi yang belum digunakan
    $prodis = Prodi::whereNotIn('id', $usedProdiId)->get();
